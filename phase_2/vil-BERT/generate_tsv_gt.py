...
def load_image_ids(split_name, group_id, total_group, image_ids=None):
    ...
    elif split_name == "customized_dataset":
      data_root = '/home/xiaoliu/nlp/dataset_merge.csv'
      with open(data_root,'rt')as f:
        data = csv.reader(f)
        image_data = {}
        for det in data:
          if det[0] not in image_data:
            image_data[det['image_id']] = []
          image_data[det['image_id']].append(det[0])
        ...
        for images in data:
          num = 0
          bbox[count, 1, 1] = images[5]
          bbox[count, 1, 2] = images[6]
          bbox[count, 1, 3] = images[7]
          bbox[count, 1, 4] = images[8]
          num += 1
          num_bbox[count] = num
          count += 1
          image_id = int(image_id)
          file_name = 'COCO_train2014_%012d.jpg' %image_id 
          filepath = os.path.join('/srv/share/datasets/customized_dataset', file_name)
          split.append((filepath,image_id))
    else:
      print('Unknown split')
